...
kk <- function(site.sf, crs.calculations) {
  ...
  site.tr <- site.sf %>% st_transform(crs.calculations)
  ...
  if(is.na(as.integer(st_intersects(site.tr, mbc.tr.centroid)))){
    ...
    mbc.tr.centroid <- st_sfc(st_cast(mbc.tr.centroid, "POINT")[[1]]) %>% 
      st_set_crs(crs.calculations)
  ...
  p <- st_set_crs(st_as_sf(st_sfc(st_point(st_coordinates(mbc.tr)[1, 1:2]))), crs.calculations)
  ...
  l <- list(mbc=mbc.json, mbc.tr=sf_geojson(mbc.tr), 
            ...
            centre=centre.json, centre.tr=centre.tr.json,
...
crs.calculations <- paste0("+proj=laea +lat_0=", yc, " +lon_0=", xc, 
                                    " +x_0=4321000 +y_0=3210000 +ellps=GRS80 ", 
                                    "+towgs84=0,0,0,0,0,0,0 +units=m +no_defs +type=crs")
crs.calculations <- paste0("+proj=aeqd +lat_0=", yc, " +lon_0=", xc, 
              " +x_0=0 +y_0=0 +R=6371000 +units=m +no_defs +type=crs")
res <- fromJSON(kk(site.sf, crs.calculations))
...
r.tr <- terra::project(r, crs.calculations)
plot(r.tr)
