from flask_api.models import SharedGeoreference, User
...
def db_create_georef(db, locationid, georef_data):
    georef = SharedGeoreference(locationid=locationid, georef_data=georef_data)
...
def db_get_georef_page(db, page, per_page):
    query = db.session.query(SharedGeoreference).order_by(
        SharedGeoreference.time_created.desc()
    )
