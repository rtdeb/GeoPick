...
cat("dif_x=", max(b1.co$X) - min(b1.co$X), ", dif_y=", max(b1.co$Y) - min(b1.co$Y), sep="")
...
mbc.co <- data.frame(st_coordinates(b1))
cat("xmin=",min(mbc.co$X), ", xmax=", max(mbc.co$X), 
    ", ymin=", min(mbc.co$Y), ", ymax=", max(mbc.co$Y), "\n", sep = "")
cat("dif_x=", max(mbc.co$X) - min(mbc.co$X), ", dif_y=", max(mbc.co$Y) - min(mbc.co$Y), sep="")
