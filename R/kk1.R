...
ggr <- function(site.sf, max_points_polygon, tolerance){
  ...
  print(st_coordinates(site.sf))
  print(st_coordinates(site.tr))
  ...
  # }
  print(st_coordinates(st_centroid(mbc.tr)))
...
site.sf <- st_as_sf(st_as_sfc("LINESTRING (0 40, 8 45, 16 40)")) %>% st_set_crs(4326)
res <- fromJSON(ggr(site.sf, 10000, 500))
print(res$centre)
